...
module.exports = (function () {
	...
	var exports = function (apiClient) {
		 ...
		 * @param {File} content
		 * @param {Object=} opts Optional parameters
		 * @param {Integer=} workers Optional parameter to tell how many workers run concurently -1: all in parallel, 1-n - default: 1 (work in serie)
		 * @param {Boolean=} retry Optional parameter to retry uploading the same file without overrides (only available with chunk upload and requires the same sessionId)
		 * @param {String=} sessionId Optional parameter to specify the Session-Id header when uploading the file in chunks. If missing the API generates one for you.
		 * @param {String[]=} opts.contentType Optional array of possible Content-Type header
		 * @param {String=} opts.contentDisposition The suggested default filename when downloading this object to a file after it has been uploaded.
		 * data is of type: {module:model/ObjectDetails}
		 * @param {Object} oauth2client oauth2client for the call
		...
		// 	if ( size <= chunkSize ) {
		// 		opts.xAdsContentSha1 = opts.xAdsContentSha1 || this._sha1(content);
		...
		// 			end: end,
		// 		};
		// 		arr.push({ range: range, sessionId: sessionId });
		// 	}

		// 	var nbWorkers = opts.workers || 1;
		// 	if (nbWorkers < -1 || nbWorkers === 0 || !nbWorkers)
		// 		nbWorkers = 1; // Force in serie

		// 	if ( nbWorkers === -1 ) {
		// 		var jobs = arr.map((item) => _this.uploadChunk(
		// 			bucketKey,
		// 			objectName,
		// 			item.range.size,
